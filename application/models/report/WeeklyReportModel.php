...
class WeeklyReportModel extends CI_Model {
	...
	public function get_report_list($search = null) {
		...
		if ($this->session->userdata('user_name') != $this->lang->line('administrator')) {
			$regist_user_ids = $this->get_user_ids();
		}
	...
	public function get_monthly_map() {
		...
		$this->db->distinct();
		$this->db->select('left(standard_date, 7) as date');
	...
	public function get_name_list() {
		...
		//0は管理者
		if ($this->session->userdata('reader') != '0') {
			$this->db->where_in('id', $this->get_user_ids());
		}
	...
	public function excel_output($id) {
		...
		$this->excel->set_cell_value('C5', $info[0]['standard_date']);
	 ...
	 * 提出回数の取得
	 */
	public function get_count_list($month) {

		$table1 = 'weekly_report';
		$table2 = 'employee';

		$this->db->select("count($table1.id) as count, $table2.name");
		$this->db->from("$table1");
		$this->db->join("$table2", "$table1.regist_user_id = $table2.id", 'left');

		$this->db->like('standard_date', $month);
		$this->db->group_by("$table1.regist_user_id");
		$query = $this->db->get();
